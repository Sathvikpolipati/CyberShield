...
function initStaticSimulation() {
    ...
    // Populate initial 8 packets
    if (packetsCache.length === 0) {
        const initProtos = ['TCP', 'UDP', 'DNS', 'HTTP', 'TCP', 'UDP', 'DNS', 'HTTP'];
        const sampleIps = ['142.250.190.46', '8.8.8.8', '1.1.1.1', '20.189.173.1', '162.159.135.232', '192.168.1.1'];
        for (let i = 0; i < 8; i++) {
            const proto = initProtos[i];
            const rip = sampleIps[i % sampleIps.length];
            const rport = proto === 'DNS' ? 53 : (proto === 'HTTP' ? 80 : 443);
            const now = new Date(Date.now() - (8 - i) * 800);
            ...
            packetsCache.unshift({
                ...
                dst_ip: rip,
                src_port: 50000 + i * 110,
                dst_port: rport,
                protocol: proto,
                length: 64 + i * 128,
                flags: 'A',
                summary: `${proto} 192.168.1.105:${50000 + i * 110} -> ${rip}:${rport} [ESTABLISHED]`,
                raw_hex_preview: '4500003c000040004006' + i + '0000'
            });
        }
    }

    // Render all initial UI components immediately
    renderPackets();
    renderAlerts();
    renderTalkers();
    renderHosts();
    renderFullSockets();
    updateMetrics(simStats, 'NORMAL');
