...
const InteractiveSynapseNetwork: React.FC<InteractiveSynapseNetworkProps> = ({
  ...
  useEffect(() => {
    ...
    class PulseImpl implements SynapsePulse {
      start: SynapseNode
      end: SynapseNode
      progress = 0
      speed = 0.03
    ...
    class NodeImpl implements SynapseNode {
      x: number
      y: number
      vx: number
      vy: number
    ...
    // initialize nodes
    nodesRef.current = Array.from({ length: nodeCount }, () => new NodeImpl())
  ...
  return (
    <div
      ...
      className={`relative w-full h-full min-h-[600px] overflow-hidden bg-[hsl(var(--network-bg))] ${className}`}
    >
      <canvas
        ref={canvasRef}
        aria-hidden="true"
